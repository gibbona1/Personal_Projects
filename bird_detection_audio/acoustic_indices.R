...
folder    <- 'ff1010bird/ff1010bird_wav/wav'
...
indices_func <- function(tmp_files, df = data.frame()){
  for(tfile in tmp_files){
    print(paste0(which(tfile == tmp_files), '/', length(tmp_files)))
...
fname <- str_split(folder, .Platform$file.sep)[[1]][1]
fname_sv <- paste0(fname, '_bioacoustic_indices.csv')
write.csv(df, fname_sv)
#df <- read.csv(fname_sv)
